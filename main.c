...
#define MM_VERSION "2.0-r196-dirty"
...
static struct option long_options[] = {
	...
	{ "min-iden",       required_argument, 0, 0 },
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "acew:k:K:t:r:f:Vv:g:I:d:XT:s:x:Hp:M:n:z:A:B:O:E:m:N:Q", long_options, &long_idx)) >= 0) {
		...
		else if (c == 'e') opt.flag |= MM_F_EXT_END;
		...
		else if (c == 0 && long_idx ==10) opt.min_iden = atof(optarg); // --min-iden
		else if (c == 'V') {
			puts(MM_VERSION);
			return 0;
		} else if (c == 'O') {
...
//		fprintf(stderr, "    -T INT       SDUST threshold; 0 to disable SDUST [%d]\n", opt.sdust_thres); // TODO: this option is never used; might be buggy
		...
		fprintf(stderr, "    -e           extend ends with DP, but no CIGAR (overridden by -a/-c)\n");
